...
if [ "$SNAPSHOT_NAME" = "$GENERATED_GENESIS_SNAPSHOT" ]; then
    ...
    if [ -f "$LOCAL_SNAPSHOT" ]; then
        ...
        if [ -z "$SNAPSHOT_NAME" ]; then
            ...
            rm -f "$LATEST_PATH"
            curl -s -o "$LATEST_PATH" "$SNAPSHOT_BUCKET/$LATEST_FILE"
            if [ ! -f "$LATEST_PATH" ]; then
                echo "Unable to fetch $SNAPSHOT_BUCKET/$LATEST_FILE"
                exit 1
            fi
        ...
        curl -s -o "$SNAPSHOT_DIR/$SNAPSHOT_FILE" "$SNAPSHOT_BUCKET/$SNAPSHOT_FILE"
    fi
