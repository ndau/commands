...
# devnet has a port offset of 50, meaning all testnet nodes start at 30X5Y where X is the service and Y is the node number.
...
for i in $( seq $START_NODE $END_NODE ); do
    ./target-group.sh $NETWORK_NAME $i --force
done
