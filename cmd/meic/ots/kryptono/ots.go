...
// An OTS is the kryptono implementation of the OTS interface
type OTS struct {
	Symbol       string
	APIKeyPath   string
	auth         Auth
...
func (e OTS) UpdateQty(order ots.SellOrder) error {
	...
	//	err := error(nil)
	err := CancelOrder(&e.auth, order.ID, e.Symbol)
	if err != nil {
...
func (e OTS) Run(
	...
	logger = logger.WithField("ots", "kryptono")

	key, err := LoadAPIKey(e.APIKeyPath)
	if err != nil {
		errs <- errors.Wrap(err, "kryptono ots: loading api key")
	}
	e.auth = NewAuth(key)
	...
	go func() {
		...
		for {
			...
			sort.Slice(orders, func(i, j int) bool {
				return orders[i].OrderPrice > orders[j].OrderPrice
			})

			for i := len(orders) - 1; i >= 0; i-- {
				if orders[i].OrderSide == "SELL" {
					fQty := fmt.Sprintf("%f", orders[i].OrderSize-orders[i].Executed)
					...
					fPrice := fmt.Sprintf("%f", orders[i].OrderPrice)
	...
	for {
		...
		if len(trades) > 0 {
			for _, trade := range trades {
				tps.Qty = tps.Qty + trade.Quantity
			}
			sales <- tps
		}
