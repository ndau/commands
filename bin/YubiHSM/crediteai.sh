...
for N in {0..4}; do
    SEQ=$(( `curl --silent --get $MAINNET/account/account/${NODES[$N]} | jq '.[].sequence'` + 1 ))
    ...
    PREVAL=`curl -s -d "$TXSIGNED" -H "Content-Type: application/json" -X POST $MAINNET/tx/prevalidate/crediteai`

    echo "Node" $N

    if [ `echo $PREVAL | jq .code` == 0 ]
    then
        curl -s -d "$TXSIGNED" -H "Content-Type: application/json" -X POST $MAINNET/tx/submit/crediteai
        echo
    else
        echo $PREVAL
    fi
