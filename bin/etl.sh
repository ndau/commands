...
# don't run ETL if there are more than 1 node
...
pushd $GO_DIR/src/github.com/oneiro-ndev/genesis || exit 1
...
# run etl, input is "output.csv" created in last step, data is written to ndau noms dir
$COMMANDS_DIR/etl
popd || exit 1
